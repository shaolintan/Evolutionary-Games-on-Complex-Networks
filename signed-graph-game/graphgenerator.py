...
#------------------------------------------------------------------------------
# generate a signed networks with a designated number of structural conflicts
def designed_signed_networks(N,s,p1,p2):
    ...
    sample=random.sample(edges,s)
    for e in sample:
        e[2]['weight']=-e[2]['weight']
    return g
    
# generate a weighted signed networks with a designated structural conflicts
def designed_weighted_signed_networks(N,s,p1,p2):
    g=nx.empty_graph(2*N)
    for i in range(N):
        for j in range(i+1,N):
            p=random.random()
...
# generate a directed signed networks with a designated structural conflicts
def designed_directed_signed_networks(N,s,p1,p2):
    g=nx.empty_graph(2*N,create_using=nx.DiGraph())
    neg_edge=0
    for i in range(N):
        for j in range(N):
            p=random.random()
