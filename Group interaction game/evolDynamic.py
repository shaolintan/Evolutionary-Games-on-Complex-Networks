...
# --special--find the fixation probability for different graphs with fixed nodes.
def DB_fixation_vs_seqgraph_fix(K,M,N):
    fix_prob1=[]
    fix_prob2=[]
    fix_prob3=[]
    fix_prob4=[]
    ...
    while i<=M:
        ...
        try:
            ...
            continue
        result1=DB_average_fixation(g,0.7,N)
        result2=DB_average_fixation(g,1.1,N)
        result3=DB_average_fixation(g,1.5,N)
        result4=DB_average_fixation(g,1.9,N)
        temp=gm.Temp_var(g)
        d_heter=gm.deg_heter(g)
        fix_prob1.append(result1)
        fix_prob2.append(result2)
        fix_prob3.append(result3)
        fix_prob4.append(result4)
        heat.append(temp)
        deg.append(d_heter)
        i=i+1
    return (fix_prob1,fix_prob2,fix_prob3,fix_prob4,deg,heat)
...
# calculate the average payoff of the whole population
def average_payoff(g,pm):
    payoff={}
    aver=0
    for d1 in g.nodes_iter():
        payoff[d1]=0
        for d2 in g.neighbors_iter(d1):
            ...
            payoff[d1]+=np.dot(mid1,[g.node[d2]['type'],1-g.node[d2]['type']])
        if payoff[d1]!=0:
            payoff[d1]=payoff[d1]/float(g.degree()[d1])
    for d in g.nodes():
        aver=aver+payoff[d]
    return aver
...
def BD_average_change_of_01edges(g,r,N,M):
    nodes=g.nodes()
    num=BD_change_of_01edges(g,r,N)
    aver_num01=num[0]
    aver_num11=num[1]
    aver_num00=num[2]
    aver_num2=num[3]
...
def BD_first_time(g,r,nbunch,N):
    ...
    while i<N:
        ...
        death=BD_random_death(g,birth)
        g=update_type(g,death[0],birth)
        if g.node[birth]['type']==1:
            if g.node[death[0]]['first_time']==N:
                g.node[death[0]]['first_time']=i
        test=test_nodes_type(g)
        if test==0 or test==1: break
    return (g,test)         
...
def BD_average_first_time(g,r,nbunch,N,M):
    ...
    while k<M:
        ...
        if rst[1]==1:
            for d in g.nodes():
                g.node[d]['average_first_time']=g.node[d]['average_first_time']+g1.node[d]['first_time']
            k+=1
    for d in g.nodes():
        g.node[d]['average_first_time']=g.node[d]['average_first_time']/float(M)
    return g

# average the first time for different initial settings
def BD_average_time(g,r,N,M):
    S=g.order()
    a={}
    for d in g.nodes():
        a[d]=0
    nx.set_node_attributes(g,'average_time',a)
    for d1 in g.nodes():
        g1=BD_average_first_time(g,r,d1,N,M)
        for d2 in g.nodes():
            g.node[d2]['average_time']=g.node[d2]['average_time']+g1.node[d2]['average_first_time']
    for d in g.nodes():
        g.node[d]['average_time']=g.node[d]['average_time']/float(S)
    return g
    
        
# Compute the averaged fraction of 0-1edges, 0-0edges and 1-1edges by mean field method
def edges_vs_mutants(g):
    size=g.order()
    nodes=g.nodes()
    ...
    mutants=[]
    deg=g.degree()
    deg=sorted(deg.items(),key=itemgetter(1),reverse=True)
    nbunch=[]
    for k in range(size):
        i=k+1
        mutants.append(i)
        m=0
        nbunch.append(deg[k][0])
        g=add_dynamic_attributes(g,nbunch)
